...
export default class StringSearch {
  ...
  constructor (list: any[], options: SearchProperties) {
    this.properties = {
      ...
      onGet: deepValue,
      // Sort results : Default sort function
      onSort: (a: SearchResult, b: SearchResult) => (a.score - b.score),
  ...
  search (pattern: string) {
    ...
    this.properties.onSort && this._sort(results);
  ...
  _search (fullSearcher: Bitap, tokenSearchers: any[]) {
    const {keys = [], onGet = deepValue} = this.properties;
  ...
  _sort (results: SearchResult[]) {
    this._log('\n\nSorting....');
    results.sort(this.properties.onSort)
  }

  _format (results: SearchResult[]) {
    const {
      verbose, id, caseSensitive, tokenSeparator = / +/g, onGet = deepValue
    } = this.properties;
        ...
        let obj: any = {
          indices: item.matchedIndices.map((range: [number, number]) => {
            ...
            if (type === 'fuzzy') {
              ...
              /* NOT WORKING --> separator "." --> "mastodon.s" = "mastodon.at"
              if (type === 'fuzzy') {
    ...
    for (let i = 0, len = results.length; i < len; i += 1) {
      ...
      if (id) { result.value = onGet(result.value, id)[0] }
