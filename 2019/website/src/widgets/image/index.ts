...
class Image<P extends ImageProperties = ImageProperties> extends RedaktorWidgetBase<P> {
	...
	protected getConstraints(): ImageConstraints {
		const {
			placeholder, background, fade = false, fadeDuration = 320, compact = false,
	...
	protected renderPlaceholder(isIntersecting = true) {
		const {
			placeholder, placeholderStyles = {}, /*placeholderClasses,*/ size,
		...
		return (fade && loadPlaceholder && hasPlaceholder) ? v('img', {
			key: 'placeholder',
			src: placeholder,
			...
			//extraClasses: placeholderClasses
		}) : null
	}
