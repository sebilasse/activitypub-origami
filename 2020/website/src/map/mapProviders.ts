...
function ORG(name: string) { return { id: '', type: 'Organization', name } }
...
export const actor = {
  ...
  NASA: {
	...
	OPENSEAMAP: {
		...ORG('OpenSeaMap'), url: 'http://www.openseamap.org/',
		summaryMap: {
	...
	OPENPTMAP: {
		...ORG('OpenPtMap'), url: 'http://www.openptmap.org/',
		summaryMap: {
...
for (let key in actor) { (<any>actor)[key].id = `https://redaktor.me/data/Map/actor_${key}` }
...
Zu den Landmerkmalen gehören Binnengewässer und überlappende Straßen sowie schattierte Reliefbilder.`
	...
	ESRI_TOPOGRAPHIC: {
		...
		}, '7a6bf0e8cb5a418085e66c0485e74d19', major.ESRI.attr1),
		tag: [TAG.TOPO],
		contentMap: {
...
Topografiekarten Stil, gestaltet zur Benutzung mit einer erhabenen Karte.`
	...
	ESRI_TERRAIN: {
		...
		}, '33064a20de0c48d2bb61efa8faca93a8', major.ESRI.attr2),
		tag: [TAG.TERRAIN],
		contentMap: {
...
Gestaltet als Basisebene für die 'World Terrain' Referenz-Ebene und hillshade.`
	...
	ESRI_NATGEO: {
		...
		}, '3d1a30626bbc46c582f148b9252676ce', major.ESRI.attr2),
...
const ProviderItems = [basemap, layer].reduce((a: any[], o: any, i) => {
	for (let key in o) {
		...
		if (!o[key].id) { o[key].id = `${dataBase}${key}` }
		if (!o[key].tag) { o[key].tag = [] }
		o[key].tag.push(!i ? TAG.BASEMAP : TAG.REFERENCELAYER);
		o[key].type = ['MapLayer', 'Note'];
		if (o[key].hasOwnProperty('hasBaseMap')) {
			if (!Array.isArray(o[key].hasBaseMap)) { o[key].hasBaseMap = [o[key].hasBaseMap] }
			o[key].hasBaseMap = o[key].hasBaseMap.map((baseKey: string) => {
        if (basemap.hasOwnProperty(baseKey) && Basemap[baseKey].hasOwnProperty('tag')) {
          if (!o[key].tag) { o[key].tag = [] }
          if (!Array.isArray(o[key].tag)) { o[key].tag = [o[key].tag] }
          if (!Array.isArray(Basemap[baseKey].tag)) { Basemap[baseKey].tag = [Basemap[baseKey].tag] }
