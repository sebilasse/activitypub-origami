...
export const Typeahead = factory(function Typeahead({
	...
	function renderMenu(toggleClosed?: any) {
		const pos: keyof typeof themedCss = (icache.get('position')||position) as any;
		return getTotal(getOptions()) === undefined && isLoading(getOptions()) ? (
			...
			<div key="menu-wrapper" classes={[
				...
				typeof icache.get('value') !== 'undefined' ? themedCss.hasValue : null,
				animated && icache.get('isOpening') ? themedCss.opening : null,
				themedCss[pos],
				// icache.get('isClosing') ? themedCss.closing : (icache.get('isOpening') ? themedCss.opening : null)
			]}>
				{renderList(toggleClosed)}
			</div>
		);
	}
								...
								<TextInput
									...
									classes={{
										'@dojo/widgets/text-input': {
											root: [themedCss.trigger, themedCss[pos]],
									...
									valid={valid}
								>
									{{ label, leading, focusContent: (theme.isJS() === false) && renderMenu() }}
								</TextInput>
							);
						}
					}}
