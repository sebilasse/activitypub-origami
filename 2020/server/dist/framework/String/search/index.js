...
class StringSearch {
    ...
    _format(results) {
        const { verbose, id, caseSensitive, tokenSeparator = / +/g, getFn = deepValue_1.default } = this.properties;
        const finalOutput = [];
        let transformers = [];
                ...
                let obj = {
                    indices: item.matchedIndices.map((range) => {
                        ...
                        if (type === 'fuzzy') {
                            ...
                            if (type === 'fuzzy') {
                                if (tokens.length === diu_1.intersection(tokens, ...haystack.split(tokenSeparator)).length) {
                                    type = 'token';
                                }
                            }
                        }
        ...
        for (let i = 0, len = results.length; i < len; i += 1) {
            ...
            const data = { value, index: output[0].index };
